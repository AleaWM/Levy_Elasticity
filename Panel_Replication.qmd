...
subtitle: "Creates Data Used in Measure the Responsiveness of the Levy to the Tax Base"
author: "AWM"
format: 
  html:
...
## Alternate sample option:
Keeps major_type == Miscellaneous taxing agencies
...
grouped_manual <- recoded_data %>% 
  ...
  filter(total_final_levy > 1) %>%
  select(year, uniqueid, everything())
#6011 observations before filter for home rule only

# remove non-HR agencies: 1263 obs remain
grouped_manual <- grouped_manual %>% filter(home_rule_ind == 1)
```

### Create panel data, Export

```{r}
# 6011 observations in grouped_manual before making data rectangular
# 1122 homerule munis after adding home_rule_ind filter
grouped_manual_has17 <- grouped_manual %>%  
  ungroup() %>%
  arrange(home_rule_ind) %>%
