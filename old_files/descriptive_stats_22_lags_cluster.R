...
#
# Core v22 changes:
# 1. Keep non-home-rule municipalities as type_2 == "Muni".
...
# ---- output helpers --------------------------------------------------------
output_dir <- "merriman_file_output_newerfile"
...
reg_df <- reg_df |>
  mutate(
    # Stata v22 leaves non-home-rule municipalities as "Muni".
    type_2 = case_when(
      type == "Muni" & home_rule_ind == 1 ~ "HR_muni",
      TRUE ~ as.character(type)
    ),
...
gov_types <- c("Other", "Township", "HR_muni", "Muni", "School")
gov_types_B <- c("HR_muni", "Muni", "School")
