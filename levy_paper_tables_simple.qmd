...
reg_df <- reg_df |>
  mutate(
    ...
    pos_d_eav = d_eav * eav_growth,   # d_eav * 1 = d_eav if positive, d_eav * 0 = 0 if negative
    neg_d_eav = d_eav * (1 - eav_growth),  # d_eav * (1-1) = 0 if positive, d_eav * (1-0) = d_eav if negative
  )
table(reg_df$pos_d_eav, reg_df$year)
...
reg_df <- reg_df |>
  mutate(
    ...
    pos_d_eav_hat2 = if_else(d_eav_hat > 0, d_eav_hat, 0),
neg_d_eav_hat2 = if_else(d_eav_hat <= 0, d_eav_hat, 0)
  )
...
```{r}
asym_iv_3 <- feols(d_levy ~ 1 | year^home_rule_for_fe + n_uniqueid | neg_d_eav + pos_d_eav ~ reassess_year + pos_d_eav_hat2,
  data = reg_df , vcov = ~ uniqueid 
)
safe_upper_bound_two(asym_iv_3)
